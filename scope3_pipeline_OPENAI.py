...
필요한 패키지 (사전 설치)
--------------------------
pip install pandas numpy openpyxl geopy openai rapidfuzz

필요한 환경변수
----------------
- OPENAI_API_KEY   : OpenAI API 키 (필수 — 품목 분류·거리 추정 등에 사용)
...
if not openai_api_key:
    raise ValueError(
        ...
        "예) export OPENAI_API_KEY=sk-...  (또는 배포 환경의 시크릿/환경변수 설정에서 지정)"
    )
...
"""
        ...
        res = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Return only one word: flight, train, bus, or car."
                },
                {
                    "role": "user",
                    "content": prompt
...
def translate_product_for_search_info(product):
    ...
    res = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "developer",
                "content": "Return only valid JSON. Select titles only from the allowed list."
            },
            {
                "role": "user",
                "content": prompt
...
def calc_category1_2(product, qty, spend, desc="", vendor="", 대분류=None, 구분=None):
    """
    C1/C2 배출량 계산.
    activity DB 검색 시 대분류/구분 필터를 함께 전달하여 EPD 매칭 정확도 향상.
    """
    qty = to_float(qty)
    spend = to_float(spend)

    if pd.notnull(product) and qty is not None:
        name, ef, unit, cat_epd, sub_epd = search_activity_ef(product, 대분류=대분류, 구분=구분)
        ef = to_float(ef)
        if ef is not None:
            emission = qty * ef
            return name, ef, emission, "Activity"

    if spend is not None:
        picked = select_primary_spend_candidate(product=product, desc=desc, vendor=vendor)
        ef = to_float(picked["ef"])
        if ef is not None and _safe_text(picked["name"]):
            emission = spend * ef
            return picked["name"], ef, emission, "Spend"
...
def convert_to_base_unit(value: float, unit_raw: str, base_type: str = "auto") -> tuple:
    ...
    반환: (환산값, 기준단위, 환산비율, 원본단위)
    """
    if value is None:
        return None, unit_raw, 1.0, unit_raw
    ...
    if base_type == "auto":
        if u in _WEIGHT_TO_KG:
            ...
            # 변환 불필요
            return value, unit_raw, 1.0, unit_raw

    if base_type == "weight":
        ratio = _WEIGHT_TO_KG.get(u)
        if ratio is None:
            return value, unit_raw, 1.0, unit_raw
        return value * ratio, "kg", ratio, unit_raw

    if base_type == "volume":
        ratio = _VOLUME_TO_L.get(u)
        if ratio is None:
            return value, unit_raw, 1.0, unit_raw
        return value * ratio, "L", ratio, unit_raw

    if base_type == "energy_nm3":
        ratio = _ENERGY_TO_NM3.get(u)
        if ratio is None:
            return value, unit_raw, 1.0, unit_raw
        return value * ratio, "Nm³", ratio, unit_raw

    if base_type == "energy_kwh":
        ratio = _ENERGY_TO_KWH.get(u)
        if ratio is None:
            return value, unit_raw, 1.0, unit_raw
        return value * ratio, "kWh", ratio, unit_raw
...
def calc_category1_2_detail(row, category):
    ...
    product_field = "품목명" if category == "Category1" else "자산명"
    # 새 v1 템플릿: 상세 품목명/서비스명(C1) / 상세 자산명(C2) 우선 사용
    product = (
        _safe_text(row.get("상세 품목명/서비스명"))
        or _safe_text(row.get("상세 자산명"))
        or coalesce_row_fields(row, [product_field, "설명/적요", "거래처명"])
    )
    desc = _safe_text(row.get("설명/적요"))
    vendor = (
        _safe_text(row.get("거래처명(선택)"))
        or _safe_text(row.get("거래처명"))
    )
    qty = coalesce_row_numeric(row, ["수량"])
    unit = _safe_text(row.get("단위"))

    fx_meta = normalize_purchase_spend_for_usepa(row, year=FX_REFERENCE_YEAR)
    spend_usd = to_float(fx_meta.get("EF적용금액(USD)"))

    # 새 v1 템플릿: 품목군(C1)/자산분류(C2) → 대분류/구분으로 EPD 검색 정밀도 향상
    _epd_대분류 = (
        _safe_text(row.get("대분류"))
        or _safe_text(row.get("품목군"))
        or _safe_text(row.get("자산분류"))
        or _safe_text(row.get("분류"))
    ) or None
    _epd_구분 = (
        _safe_text(row.get("구분"))
        or _safe_text(row.get("계정과목"))
        or _safe_text(row.get("계정과목(선택)"))
    ) or None

    name, ef, emission, method = calc_category1_2(
        product=product,
        qty=qty,
        spend=spend_usd,
        desc=desc,
    ...
    picked = select_primary_spend_candidate(product=product, desc=desc, vendor=vendor)
    group_info = classify_purchase_group(row)

    recommend_n = 4
    all_candidates = ensure_spend_candidates(
        primary_name=name,
        primary_ef=ef,
        product=product,
        desc=desc,
        vendor=vendor,
        family=group_info["추천Family"],
        top_n=recommend_n + 1
    )

    primary_candidate = all_candidates[0]
    alt_candidates = all_candidates[1:1 + recommend_n]   # 대표 1 + 추천 4 = 5개 총 확보

    # 대표 후보 기준으로 덮어씀
    rep_name = primary_candidate["title"]
    rep_ef = primary_candidate["ef"]

    # 배출량도 대표 EF 기준으로 다시 계산
    if method == "Spend" and spend_usd is not None and rep_ef is not None:
        emission = spend_usd * rep_ef
    elif method != "Spend" and qty is not None and rep_ef is not None:
        emission = qty * rep_ef

    result = {
        "구매 월": _get_purchase_month(row),
        "입력통화": fx_meta.get("입력통화"),
        "단위환산비고": fx_meta.get("단위환산비고"),
        "환율기준연도": fx_meta.get("환율기준연도"),
        "환율적용월": fx_meta.get("환율적용월"),
        "적용환율(원/USD)": fx_meta.get("적용환율(원/USD)"),
        "원화환산금액": fx_meta.get("원화환산금액"),
        "EF적용금액(USD)": fx_meta.get("EF적용금액(USD)"),
        "환율적용여부": fx_meta.get("환율적용여부"),

        "group_key": group_info["group_key"],
        "품목/자산명": group_info["품목/자산명"],
        "그룹판단근거": group_info["그룹판단근거"],
        "추천Family": group_info["추천Family"],
        "자동카테고리": group_info["자동카테고리"],

        "매개변수": rep_name,
        "EF_DB": rep_name,
        "배출계수": rep_ef,
        "배출량": emission,
        "EF_DB_추천사유": make_ef_db_reason(
            ef_db_name=rep_name,
            method=method,
            product=product,
            desc=desc,
            vendor=vendor,
            family=group_info["추천Family"],
            pick_source=picked.get("pick_source"),
        ),
        "계산방식": method,
        "계산식": (
            format_emission_formula(
                method, qty=qty, unit=unit, spend=spend_usd, ef=rep_ef, emission=emission
            )
            if method != "Spend"
            else _format_spend_formula_usd(spend_usd, rep_ef, emission)
        ),
    }
    ...
    for i in range(1, recommend_n + 1):
        ...
        result[f"추천EF{i}"] = cand["title"]
        result[f"추천EF{i}_배출계수"] = cand["ef"]
            ...
            if category in ("Category1", "Category2"):
                ...
                for _, row in df.iterrows():
                    ...
                    if usage_raw is not None:
                        conv_val, std_u, ratio, _ = convert_to_base_unit(usage_raw, unit_str, "weight")
                        usage_kg = conv_val
                        if ratio != 1.0:
                            row2["단위환산비고"] = f"{unit_str}→kg (×{ratio})"
                    else:
                        usage_kg = None
...
# C1/C2 상세 계산 override: spend_usd 대신 spend_krw 사용
def calc_category1_2_detail(row, category):
    row = row.copy()
    ...
    desc = _safe_text(row.get("설명/적요"))
    vendor = _safe_text(row.get("거래처명(선택)")) or _safe_text(row.get("거래처명"))
    qty = coalesce_row_numeric(row, ["수량"])
    unit = _safe_text(row.get("단위"))

    spend_meta = normalize_purchase_spend_for_usepa(row, year=globals().get("FX_REFERENCE_YEAR", 2025))
    spend_krw = to_float(spend_meta.get("EF적용금액(KRW)"))
    ...
    # calc_category1_2는 spend 인자를 그대로 곱하므로, 여기서는 KRW 금액을 넣는다.
    name, ef, emission, method = calc_category1_2(
        product=product,
        qty=qty,
        spend=spend_krw,
    ...
    primary_candidate = all_candidates[0]
    alt_candidates = all_candidates[1:1 + recommend_n]

    rep_name = primary_candidate["title"]
    rep_ef = primary_candidate["ef"]

    if method == "Spend" and spend_krw is not None and rep_ef is not None:
        emission = spend_krw * rep_ef
    elif method != "Spend" and qty is not None and rep_ef is not None:
        emission = qty * rep_ef

    result = {
        "구매 월": _get_purchase_month(row),
        "입력통화": spend_meta.get("입력통화"),
        "단위환산비고": spend_meta.get("단위환산비고"),
        "배출계수기준연도": spend_meta.get("배출계수기준연도"),
        "배출계수기준컬럼": spend_meta.get("배출계수기준컬럼"),
        "배출계수단위": "kgCO2e/KRW" if method == "Spend" else None,
        "원화환산금액": spend_meta.get("원화환산금액"),
        "EF적용금액(KRW)": spend_meta.get("EF적용금액(KRW)"),
        # 구버전 호환용 중간 컬럼
        "환율기준연도": spend_meta.get("환율기준연도"),
        "환율적용월": spend_meta.get("환율적용월"),
        "적용환율(원/USD)": spend_meta.get("적용환율(원/USD)"),
        "EF적용금액(USD)": spend_meta.get("EF적용금액(USD)"),
        "환율적용여부": spend_meta.get("환율적용여부"),
        ...
        "EF_DB_추천사유": make_ef_db_reason(
            ...
            vendor=vendor,
            family=group_info["추천Family"],
            pick_source=picked.get("pick_source") if isinstance(picked, dict) else None,
        ),
        "계산방식": method,
        "계산식": (
            format_emission_formula(method, qty=qty, unit=unit, spend=spend_krw, ef=rep_ef, emission=emission)
            if method != "Spend"
            else _format_spend_formula_krw(spend_krw, rep_ef, emission)
        ),
    }
...
def normalize_travel_mode(mode):
    ...
    if any(k in s for k in ["승용차", "자가용", "차량", "car"]):
        return "car"
    try:
        return detect_travel_mode_rule(mode)
    except Exception:
        return "car"
...
# 웹/서버용 진입점
# ══════════════════════════════════════════════════════════════
def run_scope3_pipeline(input_file: str, output_dir: str = ".",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", default=".", help="결과 파일 저장 폴더 (기본: 현재 폴더)")
